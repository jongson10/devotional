...
export default function ReflectionsFeed() {
  ...
  async function react(type: "AMEN" | "PRAYING", id: string) {
    await fetch("/api/reactions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, reflectionId: id }) });
  }
  return (
    <div style={{ padding: "26px 18px" }}>
      ...
      {!loading && items.length === 0 && (<div className="glass" style={{ borderRadius: 16, padding: 24, textAlign: "center", color: "var(--muted)" }}>No reflections yet. As people work through the devotional, their reflections appear here for the church to read.</div>)}
