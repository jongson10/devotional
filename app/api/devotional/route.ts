...
export async function GET(req: NextRequest) {
  ...
  const where = dayId ? { id: dayId } : seriesId && order ? { seriesId, order: Number(order) } : null;
  if (!where) return NextResponse.json({ error: "bad request" }, { status: 400 });

  const day = await prisma.day.findFirst({
    where,
    include: { series: { select: { id: true, title: true, weekNumber: true, churchId: true, published: true, startDate: true, church: { select: { timezone: true } } } } },
  });
  ...
  let esv = false;
  if (!passageText) {
    const fetched = await fetchEsvPassage(day.passageRef);
    if (fetched) { passageText = fetched; esv = true; }
  }
  ...
  return NextResponse.json({
    day: {
      id: day.id, order: day.order, title: day.title,
      passageRef: day.passageRef, passageText,
      passageRefsExtra: day.passageRefsExtra, pastorNote: day.pastorNote,
      teaching: day.teaching, reflectionQuestions: day.reflectionQuestions,
      prayerPrompt: day.prayerPrompt, pointsReward: day.pointsReward,
      seriesTitle: day.series.title, weekNumber: day.series.weekNumber,
      esv, esvCopyright: esv ? ESV_COPYRIGHT : null,
    },
    // hydration state so the flow resumes where the user left off
