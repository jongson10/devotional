...
// Church leaderboard: streak + stars (Redis) and days completed (Postgres),
// for every member of the signed-in user's church.
export async function GET() {
  ...
  // days completed per user (Postgres)
  const completedCounts = await prisma.dayProgress.groupBy({
    by: ["userId"],
    where: { user: { churchId: user.churchId }, completed: true },
    _count: { _all: true },
  });
  const doneByUser = new Map<string, number>(completedCounts.map((c: { userId: string; _count: { _all: number } }) => [c.userId, c._count._all]));

  // streak + stars per user (Redis) — gather in parallel
