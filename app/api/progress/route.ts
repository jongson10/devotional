...
const STEP_WEIGHTS = [0.17, 0.33, 0.33, 0.17]; // ~10/20/20/10 of 60
function starsForStep(step: number, total: number): number {
  let acc = 0;
  for (let i = 0; i < step && i < STEP_WEIGHTS.length; i++) acc += STEP_WEIGHTS[i];
  return Math.round(total * acc);
}
...
export async function POST(req: NextRequest) {
  ...
  const day = await prisma.day.findFirst({
    where: { id: dayId, series: { churchId: user.churchId ?? "__none__" } },
    select: { id: true, order: true, pointsReward: true, series: { select: { startDate: true, church: { select: { timezone: true } } } } },
  });
  if (!day) return NextResponse.json({ error: "forbidden" }, { status: 403 });

  const tz = day.series.church?.timezone || "UTC";
  if (user.role === "MEMBER" && !isDayOpen(day.series.startDate, day.order, tz)) {
  ...
  if (reached <= prevStep) {
    const state = await getStreak(user.id);
    return NextResponse.json({ noChange: true, step: prevStep, completed: existing?.completed ?? false, ...state });
  }
  ...
  // on-time = completing on the day's own scheduled day (admins always count as on-time preview)
  ...
  // full stars either way; streak credit only when completed ON its own day
  let state;
  if (completed && onTime) state = await recordCompletion(user.id, delta);
  else state = await addPoints(user.id, delta);
